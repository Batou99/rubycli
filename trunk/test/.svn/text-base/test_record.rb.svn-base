...
class Test_Record < Test::Unit::TestCase
  ...
  def test_integers
    using_table('ints', 'b BYTEINT, s SMALLINT, i INTEGER, q BIGINT') {|name|
  ...
  def test_DECIMAL
    using_table('decs', 'd1 DECIMAL(8,4)') {|name|
      insert name, '0.0007'
      rec = select(name).first
      assert_equal 1, rec.size
      assert_equal '0.0007', rec[:d1]
    }
  end

  def test_strings
    using_table('strs', 'c CHAR(4), vc VARCHAR(4), lvc LONG VARCHAR, b BYTE(4), vb VARBYTE(4)') {|name|
      insert name, "'ab', 'cd', 'long', '6566'XBF, '6768'XBV"
      rec = select(name).first
      assert_equal 5, rec.size
      assert_equal 'ab', rec[:c].rstrip
      assert_equal 'cd', rec[:vc].rstrip
      assert_equal 'long', rec[:lvc].rstrip
      assert_equal 'ef', rec[:b].rstrip
      assert_equal 'gh', rec[:vb].rstrip
    }
  end

  def test_strings_not_null
    using_table('strs', 'c CHAR(4) NOT NULL, vc VARCHAR(4) NOT NULL, lvc LONG VARCHAR NOT NULL, b BYTE(4) NOT NULL, vb VARBYTE(4) NOT NULL') {|name|
      insert name, "'ab', 'cd', 'long', '6566'XBF, '6768'XBV"
      rec = select(name).first
      assert_equal 5, rec.size
      assert_equal 'ab', rec[:c].rstrip
